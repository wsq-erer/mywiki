### ​House of Botcake
 给我们高版本 libc 下的 Tcache Double Free
​
...
#### 原理：
感觉就是高级的Double Free
进入tcache里面的堆块会自动生成一个key对应堆块，再次申请时会有一个key进行检查
防止在tcache里面的堆块发生Double Free
House of botcacke 合理利用了 Tcache 和 Unsortedbin 的机制
同一堆块第一次 Free 进 Unsortedbin 避免了 key 的产生，第二次 Free 进入 Tcache，让高版本的 Tcache Double Free 再次成为可能
​

### House of Roman
#### 使用条件
2.23----2.29
存在Double Free，以及   堆溢出   和  offbyone
没有show功能的时候使用
​
...
[exp_debug.py](House of 系列.assert\exp_debug.py) (2 KB)
本题目存在offbyone和double  free
name = b"A"*20
p.recvuntil(":")
p.sendline(name)
​
...
chunk_1在unsortedbin
create(0xc8,1)
将chunk_1再申请回来，由于double  free的存在，fd指针还存在
...
over += b"\x71" 
edit(0,over)
offbyone该chunk_1的size（在后面更改chunk_2地址的时候不会报错）
​
...
修改chunk_1中的main_arena+88的地址为malloc_hook-0x23的地方


然后进行申请堆块将malloc_hook-0x23地方申请出来
​

free(15)     （malloc_hook-0x23)
edit(15,p64(0x00))
info("再次生成 0x71 的 fastbin, 同时修改 fd =0, 修复 fastbin")
​

把malloc_hook堆块的size改回去
然后进行free，堆块就会有libc地址，再进行更改libc为onegadget就可以了


### house  of  froce
#### 原理
在2.23和2.27的libc版本中，由于没有对top chunk的size合法性进行检查，因此如果我们能够控制top chunk的size位以及malloc在申请堆块时的大小不受限制（条件），那么就可以完成该攻击。
公式：
 new_top的地址 到 0x404060，现在要让0x404060-0x20 =old_top的地址(0x405110) + size ,所以这个 size 应该是 0x404040-0x405110=0xffffffffffffef30
#### 使用条件
1，控制top chunk的size位（堆溢出）
2，申请堆块时不限制大小
3，2.23到2.27版本之间使用
#### 前置知识
利用realloc函数实现one_gadget的条件
##### 详细文章

> **[使用realloc函数来调整栈帧让one_gadget生效 - ZikH26 - 博客园](https://www.cnblogs.com/ZIKH26/articles/16421631.html#_label3)** — 博客园
> 使用one_gadget的时候，必须要满足一定条件，如果所有one_gadget都没有满足条件，那么我们可以使用realloc函数来调整栈帧打one_gadget。本文以2.23的libc版本中的realloc函数举例说明使用realloc函数是如何调整栈帧打one_gadget的，但是在不同的li

##### 理解
首先、realloc函数存在一个__realloc_hook，执行realloc的时候会判断__realloc_hook是否为空，如果不为空，则执行__realloc_hook指向的内容，
然后__realloc_hook和__malloc_hook的地址是挨着的  ，这就意味着我们覆写__malloc_hook的时候可以顺便控制__realloc_hook。
因此我们把__malloc_hook改成__realloc_hook然后__realloc_hook写入one_gaget，最后依然可以执行one_gadget
*主要原因**、realloc函数中有大量的**push指令**（在执行__realloc_hook之前），因此我们将realloc函数的地址加上一定的偏移，就可以选择去执行一定量的push指令，从而抬高栈帧（我指的抬高栈帧是栈帧又向着低地址增长了）。这样rsp增加了之后，我们就可以控制例如rsp+0x30，让其内存值正好落在0处。*
尽量看realloc函数在实现时的汇编
​
...
首先，申请的chunk大小不受限制，申请的chunk大小为0x2330000，会使堆块的地址挨着libc基址
![屏幕截图 2025-04-28 190219.png](./House of 系列.assert/1745838170505-e606e689-2419-402f-aa5e-5945338909a6.png)

也就可以利用该方发泄露libc基址（至少申请0x1fbfe9）
​

top = alloc(0x10, b'a' * 0x10 + p64(0) + p64(0xffffffffffffffff)) + 0x10
构造top  chunk的size位为0xffffffffffffffff，用来绕过检查，同时接收top chunk的头地址
​

addr = alloc(malloc_hook - top - 0x30, b'a')
用来使堆的地址变成自己想申请的地址，公式如下   （想申请的地址  -   top chunk  -  0x30）
![屏幕截图 2025-04-28 200618.png](./House of 系列.assert/1745842011537-3ebd83f7-3890-4f6f-898a-d217a368ccae.png)

addr = alloc(0x50, b'a' * 0x8 + p64(one_gadget) + p64(realloc + 0x10))
填充one_gadget，以及realloc，
![屏幕截图 2025-04-28 200811.png](./House of 系列.assert/1745842101447-7950e9b2-f476-476d-b2f6-e3798935147a.png)

realloc_hook里面是one_gadget
malloc_hook里面是realloc函数，
在调用malloc函数的时候会跳到realloc函数，对realloc函数进行检查，realloc函数里面存在one_gadget，进而跳转到one_gadget
​

### house  of   lore
#### 简述
利用smallbin，双链表取出堆块，其实理解了smallbin的取出堆块的原理基本就会了，一般打got表
#### 使用条件
1，got表能改
2，文件中存在数据存储（比如说：文件在读入的一些数据会进入bss段）或者能在bss段及data段里面写（用来构造smallbin链表）（也是可以写道栈上的）
3，bss   /  data  存在指向堆的地址，
4，uaf，或者  堆溢出（包括off  by  one）
5，2.23-2.27
#### 前置知识（含调试）
...
进入smallbin的条件
1，堆块大于0x80     （free后先进入unsortedbin）
2，free后再申请一个大堆块（比如0x400）
​

![屏幕截图 2025-04-29 175705.png](./House of 系列.assert/1745920637507-f0b1395b-b330-45f7-972d-7c09d8ebbec9.png)

第一个堆块进入smallbin
连续两次，两个相同大小的堆块就进入smallbin
​

看指针如下
![屏幕截图 2025-04-29 180036.png](./House of 系列.assert/1745920846299-8c4e4aaa-18df-4d96-ae62-b1c5f243baf1.png)

![屏幕截图 2025-04-29 180458.png](./House of 系列.assert/1745921118694-f1c9b3c0-dc80-4880-8b15-ee65a861aa8b.png)

定义：chunk2（A）--->chunk1（B）---->smallbin【0x100】（libc）---->chunk2（A）
chunk2的fd--->chunk1，bk--->smallbin[0x110]
chunk1的fd---->smallbin[0x110],    bk----->chunk2
smallbin[0x110]第一个指针指向chunk2，
第二个指针指向chunk1


![image.png](./House of 系列.assert/1745925901349-14e599d8-ba4a-4c50-9ce7-cf01797473cf.png)
...
那么我们就可以尝试在内存中伪造堆块（fake1与fake2共用一个bk指针）
​

当smallbin中存在     smallbin：A---->libc---->A
​

通过伪造  A  的bk指针---->  fake-0x10
               ~~~fd指针----->  任意
fake的  fd1--->A
fake的  bk1--->自身的地址（fake+0x8)
~~~~  fd2--->任意
~~~~  bk2---->fake-0x10
​
...
fd：A--->libc---->A
       fd1--->A
bk：A--->fake-0x10（B）--->bk1（libc）--->A
​

第一次申请   那么走一遍检查
​

此时fake-0x10相当于  libc   也是   B
B->bk = A            fake-0x10的  bk  为  bk1  --->A
A->bk = libc        此时fake-0x10相当于  libc   也是
...
### house  of  einherjar
无了个语的，这不就是在offbynull里面用的，来造成堆叠。
​

#### 前置知识
priv_size位记录上一个被free堆块的大小
size位的inuse位记录上一个堆块的状态
​
...
#### 前置知识
**malloc consolidate（）函数调用的条件**
**​**

** ****情况 1**：**你申请的 chunk 大小 ≥ **`**fastbin**`** 范围**
** 情况 2：top chunk 不够用了**
​
...
2. 利用largebin泄露libcbase,heapbase。泄露libcbase就是把largebin chunk free后进入unsorted bin，uaf很容易泄露libcbase。泄露heapbase就是再calloc一个比它还大的chunk让它进入largebin，覆盖它的fd,bk，show就是它的fd_nextsize，dbg一看一做差就可以了
3. largebin attack向free_hook-0x8处写入一个堆地址，这是为了绕过tcache stashing unlink attack的检查。具体做法是先让一个size大的chunk进入largebin,edit它的bk_nextsize为free_hook-0x28，再让一个size比它小的chunk先进入unsorted bin再链入largebin即可
4. 再一次largebin attack向_IO_list_all写入一个堆地址，**要记住这个堆地址，因为我们还要将它申请出来伪造FILE结构**，方法同上
5. tcache stashing unlink attack将free_hook-0x10链入0xa0的chunk大小的tcache中。让修改smallbin的第一个chunk的bk指针修改为free_hook-0x10-0x10，触发tcache stashing unlink attack。注意这里的细节，free_hook-0x8（也就是target+0x8）在之前被修改为了一个堆地址，所以可写，不会引发异常
6. 在触发tcache stashing unlink attack时，add的时候i要刚好为4，此时刚好malloc(0xe8)。**在此题中_IO_list_all写入一个堆地址是一个FAKE FILE，但是它的编写受限制，因此将其的*chain指向一个堆地址，再malloc(0xe8)刚好将这个堆地址申请出来，这里才是我们存放_IO_str_overflow的vtable的FAKE FILE!!!**
7. 在change_role中输入空字符触发len检查调用exit函数，进而执行_IO_str_overflow函数
8. exit函数会执行_IO_flush_all_lockp函数来遍历 FILE结构体，而其中就有_IO_str_overflow函数，因此要满足(fp->_mode <= 0 && fp->_IO_write_ptr > fp->_IO_write_base)才能让那个if语句执行到_IO_str_overflow
9. 在_IO_str_overflow函数中malloc,memcpy,free三连（具体细节看源码）old_blen = _IO_blen (fp);new_size = 2 * old_blen + 0x64; malloc (new_size);**注意这个malloc正是想要malloc出0xa0 chunk大小的tcache头部存的free_hook-0x10**,因此IO_buf_end，IO_buf_base，要精心设计。memcpy (new_buf, old_buf, old_blen);free (old_buf);**因此IO_buf_base要刚好是FAKE FILE中/bin/sh\x00的地址（是个堆地址）**
10. 在写exp的途中要注意修改smallbin的bk指针，largebin中的bk_nextsize指针时如果破坏了要注意修复。同时还要注意各个bin当前的状态不要和预期的状态不一样。也要注意算FILE的偏移要不要0x10这个问题
...
![屏幕截图 2025-04-24 193656.png](./House of 系列.assert/1745494627319-36115b82-7873-4399-bc09-ce181e20aef7.png)



##### 问题
###### p64（0x61）改成其他的可以吗
不可以，0x61，相当于伪造堆块在small  bin，
...
1. 为什么用  largebin  attach  往 IO_list_all  里面 写一个堆地址 就可以利用该地址就可以用来伪造  io  我也不知道，暂时知道只要地址合法就可以进行伪造，（堆地址之后就是IO_list_all的结构体）

1. 先知道利用对象是`_wide_data``_wide_vtable``_wide_vtable` 主要是没有检查，我们才进行劫持
2. ​
...
1. 调用  _IO_wifle_overflow--->_IO_wdoallcbuf--->_IO_WDOALLOCATE--->*(fp-->_wida_date-->_wida_vtable+0x68)(fp)
2.  fp --> _wide_date  --> _IO_buf_base == 0  和  fp --> _flags  &  _IO_UNBUFFERED ==0 
```plain
...
1. _flags 设置为~(2|0x8|0x800)，如果不需要控制rdi，设置为0即可；如果需要获得she11，可设置为"；sh;"
​


1. vtable设置为_IO_wfile_jumps/_IO_wfile_jumps_mmap/_IO_wfile_jumps_maybe_mmap地址（加减偏移），使其能成功调用_IO_wfile_overf1ow即可



1. _wide_data 设置为可控堆地址A，即满足*(fp + Oxa0) =A


1. awide_data->_IO_write_base 设置为0，即满足*(A + 0x18)=0_wide_data->_IO_buf_base 设置为0，即满足*(A+ 0x30)=0 


1. _wide_data->_wide_vtabTe 设置为可控堆地址日，即满足*CA +OxeO) =B


1. wide_data-_wide_vtab1e->doallocate 设置为地址C用于劫持RIP，即满足*(B+ 0x68) = C​

#### 脚本
附件：C:\Users\liuws\Desktop\pwn\堆\house   of   学习\高版本libc调试
...
将wide_vtable_add - 0x68 指向 ROP链子
​

#### ORW  之  svcudp_reply
C:\Users\liuws\Desktop\pwn\轩辕杯\附件   为例第一种写法
![a4d62fcc4961311f0b155aed89e05d25.png](./House of 系列.assert/1757575392188-8d648831-c343-430b-b3fd-fa6a65bc1d8b.png)
...
```
##### orw出现的错误
###### 一  伪造_IO_save_base时出现错误
_IO_save_base应该为orw中第一个pop所在的地址减8出
![image.png](./House of 系列.assert/1757575756821-81fd9bb9-e306-4edb-914a-15492b4825cc.png)

rbp  变成  rdi+0x48所在存放的地址即_IO_save_base  因为要进行一次栈迁移，所有需要在~~~~
###### 二   leave_ret所在的位置放错
![image.png](./House of 系列.assert/1757576366199-40c847e6-4dd3-4ffb-bc07-ddbbda4885d2.png)

ni之后
![image.png](./House of 系列.assert/1757576314525-0be236b2-0bb1-4b71-9206-e000fd862d05.png)

发生了变化，在rbp+0x10的地址内容发生了变化，从而导致不能正常进行leave_ret
### orw  之  setcontext
![C:\Users\wsqni\Downloads\lakebook-to-md-main\123\House of 系列/House of 系列.assert/1758164615786-691cebfb-f680-4e81-82cf-37d3e8f7abe1.png](./House of 系列.assert/1758164615786-691cebfb-f680-4e81-82cf-37d3e8f7abe1.png)
...
```
![屏幕截图 2025-09-18 110146.png](./House of 系列.assert/1758164525322-d6b3679c-c113-4f21-9328-502059510892.png)
...
3. 当stdout在bss段上时，是不能直接伪造_IO_2_1_stdout_注：暂时看到的是需要将_IO_2_1_stdout_所在的地址当作堆块申请出来，然后进行改动
#### 触发条件

- **flags = 0x00008000**
- **_IO_backup_base > _IO_save_base (1>0)**
- **无需设置 _lock字段为可写地址**
- **mode = 0**#### stdout在bss段上时
...
```
##### ~~利用largebin将stdout改成堆地址（以下伪造io只适用于2.23 因为存在~~`~~_IO_vtable_check~~`）    错啦
伪造需要先进行输入
```plain
fake_io = b""       
...
```
注：可以将调表改成_IO_str_jump里面的，_IO_wfile_jumps（不可以） 利用宽字符，但是在程序进入 puts函数之后 mode  会变成  -1  这也就意味着部分利用puts刷新io的程序是不行的  同时在进行_overflow的时候会卡住
 利用apple2 时 ，会出现的情况 ， IO_write_ptr  <  IO_write_base的情况，且不可进行堆风水来调控，
![屏幕截图 2025-05-23 203752.png](./House of 系列.assert/1748003895420-34070657-a4c1-444b-b164-bfc86e5bc1a3.png)
...
【强网杯2022&pwn&house_of_cat】[https://www.bilibili.com/video/BV1XV4y1j7kf?vd_source=366ddcfb960bc44528433ecade6fa3f6](https://www.bilibili.com/video/BV1XV4y1j7kf?vd_source=366ddcfb960bc44528433ecade6fa3f6)


#### 前置知识

1. house  of  kiwi/emma
2. 修改  stderr/_IO_list_all 
3. 触发条件：FSOP有三种情况（能从main函数中返回、程序中能执行exit函数、libc中执行abort），第三种情况在高版本中已经删除;__malloc_assert则是在malloc中触发，通常是修改top chunk的大小。
4. 调用链:**__malloc_assert-> __fxprintf->__vfxprintf->locked_vfxprintf->__vfprintf_internal->_IO_wfile_seekoff->_IO_switch_to_wget_mode->setcontext->orw**
#### 脚本
...
> **[House Of Emma-原理](https://ywhkkx.github.io/2022/08/07/House%20Of%20Emma-%E5%8E%9F%E7%90%86/)** — Pwn进你的心
> House Of Emma在 libc-2.34 版本中，常用的 hook 都被 ban 掉了（malloc_hook，free_hook 等），我们要尝试通过控制 IO 来获取 shell 应用场景是： 可以任意写一个可控地址（LargeBin Attack，Tcache Stashing Unlink Attack） 可以触发 IO 流（FSOP，House Of Kiwi） House


> **[奇安信攻防社区-libc2.34下的堆利用--House_of_emma分析](https://forum.butian.net/share/1691)**
> 奇安信攻防社区-libc2.34下的堆利用--House_of_emma分析


> **[house of emma利用手法详解（21湖湘杯实例解析） - FreeBuf网络安全行业门户](https://www.freebuf.com/articles/system/348256.html)** — freeBuf
> house of emma利用分析，利用链跟踪，21湖湘杯house of emma题目讲解
...
1. house  of  obstack
> **[GLIBC2.36利用obstack去劫持执行流 - 何思泊河 - 博客园](https://www.cnblogs.com/trunk/p/17338793.html)** — 博客园
> # GLIBC2.36中利用obstack去劫持执行流 &gt; 作者没有起名字，可能就是跟house of apple太相似了 ，就是roderick师傅提出的house of apple中没有发现的一个链，个人感觉就是house of apple跟house of banana的一个结合(说实话

[https://zikh26.github.io/posts/2ddfe893.html](https://zikh26.github.io/posts/2ddfe893.html)
​

environ  放着 栈地址
#### 2.39  apple2 之 shellcode
...
![image.png](./House of 系列.assert/1761223733111-75e7c3d2-86c9-43dd-ba0d-91203e682de7.png)

![image.png](./House of 系列.assert/1761223855094-3d1b3f78-122d-44a8-81c7-65a5813fa38b.png)
